...
local function getbiasloc2(daynum, dayfrc)
  ...
  local function chkutc()
    tml.isdst = nil; local tvug = ostime(tml)
    if tvug and (tvl == tmtotv(osdate("*t", tvug))) then
      tvu = tvug
      return
    end
    tml.isdst = true; local tvud = ostime(tml)
...
function strwalker:__call(s, f)   -- print("strwalker:__call "..s..self:aimchr())
  ...
  if is then
    self.e, self.i = self.i, 1 + ie; if f then f(unpack(self)) end
    return self
  end
end
...
local function date_parse(str)
  ...
  local function setzn(zs, zn)
    zn = tonumber(zn); setz(((zn < 24) and (zn * 60) or (mod(zn, 100) + floor(zn / 100) * 60)) * (zs == '+' and -1 or 1))
  end
  ...
  if not (sw("^(%d%d%d%d)", sety) and (sw("^(%-?)(%d%d)%1(%d%d)", function(_, a, b)
          setm(tonumber(a)); setd(tonumber(b))
        end) or sw("^(%-?)[Ww](%d%d)%1(%d?)", function(_, a, b) w, u = tonumber(a), tonumber(b or 1) end) or sw("^%-?(%d%d%d)", setj) or sw("^%-?(%d%d)", function(
            a)
          setm(a); setd(1)
        end))
  ...
  if (not y and not h) or ((m and not d) or (d and not m)) or ((m and w) or (m and j) or (j and w)) then return error_inv(
    "!") end
  -- fix month
  if m then m = m - 1 end
...
local function date_fromtable(v)
  ...
  date_new(y and makedaynum(y, m, d) or DAYNUM_DEF, makedayfrc(h or 0, r or 0, s or 0, t or 0))
end
local tmap = {
  ...
  ['table']   = function(v)
    local ref = getmetatable(v) == dobj; return ref and v or date_fromtable(v), ref
  end
}
...
function dobj:getfracsec() return mod(floor(self.dayfrc / TICKSPERSEC), SECPERMIN) +
  (mod(self.dayfrc, TICKSPERSEC) / TICKSPERSEC) end
...
local tvspec = {
  ...
  ['%z'] = function(self)
    local b = -self:getbias(); local x = abs(b); return fmt("%s%.4d", b < 0 and "-" or "+",
...
function dobj:fmt0(str) return (gsub(str, "%%[%a%%\b\f]", function(x)
    local f = tvspec[x]; return (f and f(self)) or x
  end)) end

function dobj:fmt(str)
  str = str or self.fmtstr or fmtstr
  return self:fmt0((gmatch(str, "${%w+}")) and
...
function date:__call(arg1, ...)
  ...
  elseif arg_count == 0 then
    ...
    local o, r = date_getdobj(arg1); return r and o:copy() or o
  end
end
...
function date.fmt(str)
  if str then fmtstr = str end; return fmtstr
end
